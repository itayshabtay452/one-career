...
הפרויקט נמצא בשלב 1: הקמת שלד האפליקציה וה־Vertical Slice הראשון. שלד ה־Next.js PWA ומנוע הקריירה הדטרמיניסטי קיימים, ופרוסת שלוש העונות נבנית במסגרת Issue #5. המצב המדויק והמשימה הפעילה מתועדים ב־[docs/STATUS.md](docs/STATUS.md).
...
- `src/app` — מסכי App Router, מטא־דאטה ורישום ה־service worker. `src/app/play` הוא מסלול הפרוסה.
